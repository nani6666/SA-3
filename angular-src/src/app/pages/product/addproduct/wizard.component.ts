...
export class WizardComponent {
  ...
  validationMessages = {
    ...
    'ProductionCapacity': {
      ...
      'pattern': 'ProductionCapacity should be positive number.'
    },
    'CompanyProductId': {
  ...
  constructor(private http: Http, private formBuilder: FormBuilder, private wizardApiService: WizardApiService, private router: Router, public toastrService: ToastrService) {
    ...
    this.productForm = this.formBuilder.group({
      ...
      'ProductionCapacity': ['', [Validators.required, Validators.pattern('^[+]?[0-9]+')]],
  ...
  public deleteTradeDetailForm(data) {
    ...
    this.wizardApiService.deleteTradeDetailForm(obj).subscribe((data2) => {
      if (data2.message === 'Success') {
        this.getIncoTermCostDetail();
        this.toastrService[this.types[0]]('Incoterm Deleted successfully', 'Incoterm', opt);

        this.tradeDetailOption.IncoTerms.Selected.length = this.tradeDetailOption.IncoTerms.Selected.length - 1;
      }
    });
  }
  ...
  public saveIncoTermCostDetailAdd() {
    ...
    if (this.editBtn) {
      /* this.wizardApiService.updateIncoTermCostDetail(obj).subscribe((data) => {
        this.getIncoTermCostDetail();
      ...
      this.wizardApiService.deleteTradeDetailForm(obj2).subscribe((data2) => {
        console.log(data2);
        if (data2.message === 'Success') {
          this.saveIncTrm(obj);
        }
  ...
  public getOneIncoterm(data) {
    ...
    this.wizardApiService.getOneIncoterm(obj).subscribe((data2) => {
  ...
  public saveTradeDetails() {
    console.log(this.tradeDetailOption.IncoTerms.Selected.length)
  ...
  public saveProductKeywords() {
    ...
    const obj = { ProductKeywords: this.ProductKeywordsForm.value };
